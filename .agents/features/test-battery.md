...
- **Test Request Settings** (`model.TestRunRequest`):
  ...
  - `maxRequestsPerEndpoint` (`int`): Optional request count used by `rateLimit`, capped by the validator safety limit.
...
3. **Audit Execution**:
   - Processes each job sequentially in the current implementation:
     - **Mutation**: Mutates request properties depending on the `bendType`:
       ...
       - `rateLimit`: Sends a baseline request, a controlled bounded-parallel burst, and a post-burst comparison request.
